...
class Create_NCC:
    ...
    def create(self):
        mancc = self.mancc_entry.get()
        tenncc = self.tenncc_entry.get()
        sdt = self.sdt_entry.get()
        email = self.email_entry.get()
        mst = self.mst_entry.get()
        dc = self.dc_entry.get()
        hd = self.hd_combobox.get()
        if not mancc or not tenncc or not sdt or not email or not mst or not dc:
    ...
    def create_ui(self, parent_frame):
        ...
        mancc_label = tk.Label(user_info_frame, text="Mã NCC")
        mancc_label.grid(row=0, column=0, padx = 10)
        self.mancc_entry = tk.Entry(user_info_frame)
        self.mancc_entry.grid(row=1, column=0, padx = 10)

        tenncc_label = tk.Label(user_info_frame, text="Tên NCC")
        tenncc_label.grid(row=0, column=1)
        self.tenncc_entry = tk.Entry(user_info_frame)
        self.tenncc_entry.grid(row=1, column=1)
